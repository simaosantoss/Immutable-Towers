...
data Inimigo = Inimigo
  { -- | Posição do inimigo no mapa.
    ...
    -- | Para auxiliar no caminho que o inimgo irá.
    pathIndex :: Int
  }
  deriving (Eq, Show)
...
data Jogo = Jogo
  { -- | Base de operações do jogador.
    ...
    lojaJogo :: Loja,
    -- | Índice da torre atualmente selecionada pelo jogador.
    torreSelecionada :: Int,
    -- | Número de ondas restantes antes do fim do jogo.
    ondasRestantes :: Int, 
    -- | Caminho no mapa pelo qual os inimigos seguem, definido por uma sequência de coordenadas.
    caminhoJogo :: [(Int, Int)]
  }
  deriving (Eq, Show)
